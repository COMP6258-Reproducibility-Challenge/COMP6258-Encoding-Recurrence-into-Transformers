...
#         self.theta = torch.tensor([np.pi/4], requires_grad=True)
#         self.nu = torch.tensor([np.random.uniform(1, 2)], requires_grad=True)
#         interval_1 = np.random.uniform(-2, -1)
#         interval_2 = np.random.uniform(1, 2)
#         chosen_val = float(np.random.choice([interval_1, interval_2]))
#         self.eta = torch.tensor([chosen_val], requires_grad=True)

#         k1, k2, k3, k4, k5, k6 = self.k_rem_indexes
#         d = self.dilated_factors.copy() if self.dilated_factors else []
#         self.rems = REM(k1=k1, k2=k2, k3=k3, k4=k4, k5=k5, k6=k6, d=d, truncation=n_top)
...
#         #!
#         rems = rems(eta=self.eta, nu=self.nu, theta=self.theta, query_len=qlen)
#         attn_prob = (1-F.sigmoid(self.mu)) * F.softmax(attn_scores, dim=1) + F.sigmoid(self.mu) * rems
...
class RSARelMultiHeadAttn(nn.Module):
    def __init__(self, n_head, d_model, d_head, dropout, dropatt=0,
        ...
        self.nu = nn.Parameter(torch.tensor([np.random.uniform(1, 2)], requires_grad=True))
        interval_1 = np.random.uniform(-2, -1)
        interval_2 = np.random.uniform(1, 2)
        chosen_val = float(np.random.choice([interval_1, interval_2]))
        self.eta = nn.Parameter(torch.tensor([chosen_val], requires_grad=True))
...
class RSARelPartialLearnableDecoderLayer(nn.Module):
    def __init__(self, n_head, d_model, d_head, d_inner, dropout,
                 **kwargs):
        super(RSARelPartialLearnableDecoderLayer, self).__init__()

        self.dec_attn = RSARelPartialLearnableMultiHeadAttn(n_head, d_model,
                            d_head, dropout, **kwargs)
        self.pos_ff = PositionwiseFF(d_model, d_inner, dropout, 
                                     pre_lnorm=kwargs.get('pre_lnorm'))
...
class MemTransformerLM(nn.Module):
    def __init__(self, n_token, n_layer, n_head, d_model, d_head, d_inner,
        ...
        #!
            ...
            for i in range(n_layer):
                self.layers.append(
                    RSARelPartialLearnableDecoderLayer(
                        n_head, d_model, d_head, d_inner, dropout,
                        tgt_len=tgt_len, ext_len=ext_len, mem_len=mem_len,
                        dropatt=dropatt, pre_lnorm=pre_lnorm, 
